...
function NewCampaign({ onBack, onCreated, initialEvent = null }) {
  ...
  const copyAudience = async () => {
    try {
      ...
      setBusy(true); setError(''); setMessage('')
      const result = await rallyApi.copyCampaignAudience(createdCampaign.id, sourceCampaignId)
      setAttendees(result.attendees ?? [])
      setAttendeePage(0)
      setMessage(`${result.imported} attendees copied from ${result.sourceCampaign?.name || 'the selected campaign'}. Their outreach status has been reset for this campaign.`)
  ...
  const reusableAudiences = eventCampaigns.filter((item) => item.eventId === createdCampaign?.eventId && item.id !== createdCampaign?.id && item.attendeeCount > 0)
...
function Operations({ campaign, details, onCampaignUpdated, onRefresh, onDeleted }) {
  ...
  const [collectionSettings, setCollectionSettings] = useState({ attendanceEnabled: campaign.attendanceEnabled !== false, parkingEnabled: campaign.parkingEnabled === true, foodEnabled: campaign.foodEnabled === true, sessionSlots: (campaign.sessionSlotOptions ?? []).join('\n') })
  const [callWindow, setCallWindow] = useState(defaultCallWindow)
  const [allowedSchedule, setAllowedSchedule] = useState({ allowedStartTime: '09:00', allowedEndTime: '18:00', allowedDays: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] })
  ...
  const collectionPayload = () => ({ attendanceEnabled: collectionSettings.attendanceEnabled, parkingEnabled: collectionSettings.parkingEnabled, foodEnabled: collectionSettings.foodEnabled, sessionSlotOptions: collectionSettings.sessionSlots.split('\n').map((value) => value.trim()).filter(Boolean) })
  const openLaunchPlanner = () => { setActionError(''); setNotice(''); setLaunchStep(1); setCollectionSettings({ attendanceEnabled: campaign.attendanceEnabled !== false, parkingEnabled: campaign.parkingEnabled === true, foodEnabled: campaign.foodEnabled === true, sessionSlots: (campaign.sessionSlotOptions ?? []).join('\n') }); setCallWindow(defaultCallWindow()); setShowLaunchPlanner(true) }
...
function Waitlist({ campaign, recovery, onRefresh }) {
  ...
  return <section><Header kicker="Waitlist recovery" title="Keep every released seat moving" description="When a declined attendee has no named substitute, Rally returns their reserved seat to the next consented person in this queue." action={<Button variant="secondary" onClick={recover} disabled={busy}>{busy ? 'Checking…' : 'Reconcile recovery'}</Button>} /><div className="recovery-stats"><Stat number={summary.assigned ?? 0} label="confirmed seats" /><Stat number={summary.reserved ?? 0} label="reserved" /><Stat number={summary.pendingOffers ?? 0} label="offers awaiting reply" /><Stat number={summary.queued ?? 0} label="still waitlisted" /></div><div className="steps">{['Decline recorded', 'Seat released', 'Next person called', 'Seat confirmed'].map((x,i) => <div key={x}><span>Step {i + 1}</span><b>{x}</b><p>{['A missing substitute releases the original reservation.', 'Rally safely reserves it for one waitlisted attendee.', 'Sarvam receives a recovery call request immediately.', 'Acceptance updates the event roster and capacity.'][i]}</p></div>)}</div>{error && <div className="error-banner">{error}</div>}{message && <div className="notice">{message}</div>}<div className="waitlist-note"><b>How it works</b><span>Only opted-in attendees with a phone number can receive an automatic recovery call. Each offer holds one seat until its expiry; a decline or expiry returns that seat to the next person.</span></div><h3 className="section-title">Recovery queue</h3>{rows.length ? <div className="waitlist-table"><table><thead><tr><th>Rank</th><th>Attendee</th><th>Seat</th><th>Offer status</th><th>Call delivery</th><th>Expires</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{row.rank ?? '—'}</td><td><b>{row.name}</b><br/><span>{row.optedIn ? 'Phone contact consented' : 'No call consent'}</span></td><td>{row.seatNumber ? `Seat ${row.seatNumber}` : 'Waiting for release'}</td><td><Tag type={statusClass(displayStatus(row.status))}>{displayStatus(row.status)}</Tag></td><td>{row.callFailureReason ? <span className="delivery-failed">Needs retry</span> : row.callRequestedAt ? `Requested ${formatTime(row.callRequestedAt)}` : String(row.status).toUpperCase() === 'OFFERED' ? 'Queued for Sarvam' : 'Not called'}</td><td>{row.expiresAt ? formatDateTime(row.expiresAt) : '—'}</td></tr>)}</tbody></table></div> : <div className="sarvam-results-empty">There are no waitlisted attendees in this campaign.</div>}</section>
}
